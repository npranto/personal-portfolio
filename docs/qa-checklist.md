...
- [ ] Empty state appears when no project results
- [ ] Content section shows blog posts and video thumbnails
...
The following pages are planned but do not exist yet. These checklists are for reference when those pages are built:

<details>
<summary>Projects Page (<code>/projects</code>) — not yet built</summary>
...
<details>
<summary>Project Detail Page (<code>/projects/[slug]</code>) — not yet built</summary>
...
<details>
<summary>Experience Page (<code>/experience</code>) — not yet built</summary>
...
<details>
<summary>Skills Page (<code>/skills</code>) — not yet built</summary>
...
<details>
<summary>Blog Page (<code>/blog</code>) — not yet built</summary>
...
- [ ] Home page is usable on 375px viewport (iPhone SE)
...
- [ ] `npm run validate-content` exits 0 (requires `src/scripts/validate-content.mjs` to be created first)
...
- [ ] Home page has a unique `<title>`
- [ ] Home page has a `<meta name="description">`
- [ ] Sitemap at `/sitemap.xml` is accessible
- [ ] JSON-LD on home page (Person + WebSite)
...
- [ ] `npm run check` passes (note: validate-content step will fail until script is created)
- [ ] `npm run typecheck && npm run lint && npm run build` passes (use this until scripts exist)
