...
A personal portfolio website to showcase projects, work experiences and blog posts. See it [live](https://npranto.dev/).
...
4. Start up local development server
   ...
   npm run dev
   ```
   _Note: You will need to set up a `.env` file with a few environment variables. Refer to `.env.example` in the root for all required environment variable examples._
...
4. Before committing all changes:
   1. Ensure linting passes via the following command:
      - `npm run lint`
